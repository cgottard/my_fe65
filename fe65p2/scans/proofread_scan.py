...
#		voltages = [2.0]
		path = "/home/carlo/fe65_p2/firmware/ise/SPI_bits/"
		self.bitfiles = ["fe65p2_mio_3MHz.bit", "fe65p2_mio_4MHz.bit","fe65p2_mio_6MHz.bit", "fe65p2_mio_8MHz.bit", "fe65p2_mio_12MHz.bit","fe65p2_mio_16MHz.bit","fe65p2_mio_24MHz.bit","fe65p2_mio_32MHz.bit"]
...
#					print "in position ", j, "value ", glob_errors[j]	#if you want to know where is the error
				...
				if len(errors[i]) > 0:
					...
					for i in range(0, len(errors)):
						logging.warning("iteration "+str(i)+" errors "+str(len(errors[i]))) #, " at ", ' '.join([str(x) for x in errors[i]])
